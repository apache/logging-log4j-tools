...
You need to use `ChangelogExporter`, which is accessible through the following goals of xref:../log4j-changelog-maven-plugin/README.adoc[the Maven plugin]:

`log4j-changelog:export`::
It exports the entire changelog directory (e.g., `src/changelog`) using `.index.adoc.ftl` and `.changelog.adoc.ftl` template files.

`log4j-changelog:export-single`::
It exports a single release changelog directory against a particular template file.
For instance, you can use it to export `src/changelog/2.19.0` using `src/changelog/.changelog-email.txt.ftl` to `/tmp/changelog-email.txt`.
...
. Populate the `<changelogDirectory>/<releaseVersion>` directory (e.g., `/src/changelog/2.19.0`) from the upcoming release changelog directory (e.g., `<changelogDirectory>/.2.x.x`) using the xref:../log4j-changelog-maven-plugin/README.adoc#release[`release` Maven goal]:
+
[source,bash]
----
